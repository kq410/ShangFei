...
def main():
    ...
    # adding search decision strategy
    task_start_list = [values for key, values in task_starts.items()]
    #print(task_start_list)
    model.AddDecisionStrategy(
        task_start_list, cp_model.CHOOSE_LOWEST_MIN, cp_model.SELECT_MIN_VALUE)
    ...
    if status == cp_model.OPTIMAL:
        ...
        print('model is feasible but not optimal')
    else:
        print('model is not feasible')

    solver_finish = int(time.time())
    ...
    '%s_sol_after_%i_s_with_%i_threads.xlsx'%(status_string,
    solver_finish - solver_start, solver.parameters.num_search_workers)
    )
    # else:
    ...
    [makespan, task_starts, task_ends], 20
    )
